...
export function hideAuthLoader() {
  ...
  if (window.gsap) {
    gsap.to(el, { opacity: 0, duration: 0.3, ease: 'power2.in', onComplete: _hide });
  } else {
    _hide();
  }
}
  ...
  if (window.gsap) {
    gsap.to(msgEl, {
      ...
      onComplete: () => {
        ...
        gsap.fromTo(msgEl, { opacity: 0, y: 5 }, { opacity: 1, y: 0, duration: 0.18 });
      },
    });
  } else {
    msgEl.textContent = msg;
...
export function transitionToApp() {
  ...
  if (!app) {
    console.error('[AUTH UI] #mainApp not found in DOM');
    return;
  }

  // Set display synchronously so the app is visible immediately
  if (auth) auth.style.display = 'none';
  app.style.display = 'block';

  // GSAP for polish — purely cosmetic, not structural
  if (window.gsap) {
    gsap.fromTo(app,
      { opacity: 0, y: 16 },
  ...
  console.log('[AUTH UI] Transitioned to main app');
}
...
export function transitionToAuth() {
  ...
  console.log('[AUTH UI] Transitioned to auth screen');
}
...
export function renderLoginCard() {
  ...
  if (!container) {
    console.error('[AUTH UI] #authScreen element not found in DOM');
    return;
  }

  console.log('[AUTH UI] Rendering login card into #authScreen');

  const hasRealClerk = CONFIG.CLERK_PUBLISHABLE_KEY && !CONFIG.CLERK_PUBLISHABLE_KEY.includes('REPLACE');
...
function _animateCardEntrance() {
  ...
  gsap.timeline()
    ...
    .from('.auth-card__brand',        { opacity: 0, y: 8,  duration: 0.28 }, '-=0.28')
    .from('.auth-card__title',        { opacity: 0, y: 14, duration: 0.32, ease: 'power3.out' }, '-=0.22')
    .from('.auth-card__sub',          { opacity: 0, y: 8,  duration: 0.28 }, '-=0.22')
    .from('.auth-btn--github',        { opacity: 0, y: 8,  duration: 0.24 }, '-=0.14')
    .from('.auth-divider',            { opacity: 0,        duration: 0.2  }, '-=0.1')
    .from('.auth-email-form',         { opacity: 0, y: 6,  duration: 0.22 }, '-=0.1')
    .from('.auth-card__footer-note',  { opacity: 0,        duration: 0.18 }, '-=0.1');
}
...
function _showOtpScreen(email) {
  ...
  if (window.gsap && emailForm) {
    gsap.to(emailForm, {
      ...
      onComplete: () => {
        ...
        if (otpForm) {
          ...
          gsap.fromTo(otpForm, { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.32, ease: 'back.out(1.2)' });
          gsap.from('.otp-input', { scale: 0.7, opacity: 0, stagger: 0.06, duration: 0.28, ease: 'back.out(1.6)', delay: 0.1 });
    ...
    if (otpForm)   { otpForm.style.display = 'block'; otpForm.removeAttribute('aria-hidden'); }
    document.querySelector('.otp-input')?.focus();
  }
}
    ...
    gsap.to(otpForm, {
      ...
      onComplete: () => {
        ...
        if (emailForm) {
          ...
          gsap.fromTo(emailForm, { opacity: 0, y: -8 }, { opacity: 1, y: 0, duration: 0.28 });
        }
      },
    });
  } else {
...
function _bindOtpInputs() {
  ...
  inputs.forEach((input, i) => {
    input.addEventListener('input', () => {
      ...
      if (input.value && window.gsap) gsap.from(input, { scale: 1.2, duration: 0.18, ease: 'back.out(2)' });
    });
...
export function renderUserPill() {
  ...
  if (window.gsap) gsap.from('#userPill', { opacity: 0, x: 20, duration: 0.4, ease: 'back.out(1.3)' });
...
export function clearUserPill() {
  ...
  if (!container) return;
  if (window.gsap) {
    gsap.to('#userPill', { opacity: 0, x: 16, duration: 0.22, onComplete: () => { container.innerHTML = ''; } });
  } else {
    container.innerHTML = '';
  }
}